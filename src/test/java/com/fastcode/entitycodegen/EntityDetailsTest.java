...
public class EntityDetailsTest {
	...
	public void getFields_parametersAreValid_returnFieldsList() throws IOException, ClassNotFoundException
	{
		...
		int i=0;
		for(FieldDetails obj : actual)
		{
			if(!obj.getFieldName().contains("$"))
			{
				Assertions.assertThat(obj.getFieldName()).isEqualTo(expected.get(i).getFieldName());
				Assertions.assertThat(obj.getFieldType()).isEqualTo(expected.get(i).getFieldType());
				Assertions.assertThat(obj.getIsPrimaryKey()).isEqualTo(expected.get(i).getIsPrimaryKey());
				Assertions.assertThat(obj.getIsNullable()).isEqualTo(expected.get(i).getIsNullable());
				i++;
			}
		}
	...
	public void findOneToManyJoinColFromChildEntity_entityHasJoinColumnsAnnotation_returnMap() throws ClassNotFoundException, IOException
	{
		...
		relationMap.put("Cust-Ord", relationDetails);
	...
	@Test
	public void findOneToOneJoinColFromChildEntity_entityHasJoinColumnAnnotationAndIsParentIsTrue_returnMap() throws ClassNotFoundException, IOException
	{
		ArrayList<Class<?>> classes = loadClasses();
		...
		relationMap.put("Users-Addresses", relationDetails);
	...
	@Test
	public void findOneToOneJoinColFromChildEntity_entityHasJoinColumnAnnotationAndIsParentIsFalse_returnMap() throws ClassNotFoundException, IOException
	{
		ArrayList<Class<?>> classes = loadClasses();
	...
	public void retreiveEntityFieldsAndRships_oneToManyRelationWithJoinColumnAnnotation_returnEntityDetails() throws ClassNotFoundException, IOException
	{
		...
		Assertions.assertThat(actual.getIdClass()).isEqualTo(details.getIdClass());

		for(Map.Entry<String, FieldDetails> obj : actual.getFieldsMap().entrySet())
		{
			if(!obj.getKey().contains("$"))
			{
				Assertions.assertThat(obj.getValue().getFieldName()).isEqualTo(sortedMap.get(obj.getKey()).getFieldName());
				Assertions.assertThat(obj.getValue().getFieldType()).isEqualTo(sortedMap.get(obj.getKey()).getFieldType());
				Assertions.assertThat(obj.getValue().getIsPrimaryKey()).isEqualTo(sortedMap.get(obj.getKey()).getIsPrimaryKey());
				Assertions.assertThat(obj.getValue().getIsNullable()).isEqualTo(sortedMap.get(obj.getKey()).getIsNullable());
	...
	public void retreiveEntityFieldsAndRships_ManyToOneRelationWithJoinColumnsAnnotation_returnEntityDetails() throws ClassNotFoundException, IOException
	{
		...
		EntityDetails actual = entityDetails.retreiveEntityFieldsAndRships(classes.get(5), "com.nfin.test.domain.model.Temp.Ord", classes );
		Assertions.assertThat(actual.getRelationsMap()).isEqualToComparingFieldByFieldRecursively(details.getRelationsMap());
		Assertions.assertThat(actual.getEntityTableName()).isEqualTo(details.getEntityTableName());
		Assertions.assertThat(actual.getIdClass()).isEqualTo(details.getIdClass());
	...
	public ArrayList<Class<?>> loadClasses() throws ClassNotFoundException, IOException
	{
